<div align="center">

<img src="docs/SubFlow.ico" alt="SubFlow" width="120">

# SubFlow

**Editor de subtítulos con transcripción automática — local, gratis, sin API keys.**

[**🌐 Sitio web**](https://ylimecc.github.io/SubFlow/) · [**⬇ Descargar el .exe**](https://github.com/ylimecc/SubFlow/releases/latest)

</div>

---

## ¿Qué es SubFlow?

Una aplicación de escritorio para Windows que **transcribe video/audio automáticamente** usando Whisper de OpenAI corriendo localmente, y te deja **editar los subtítulos** con una interfaz fluida y profesional — gratis y sin enviar nada a la nube.

## Características

- 🎙 Transcripción automática local con Whisper (sin API keys, sin internet)
- ⚡ Edición fluida con un clic
- 🎯 Segmentación inteligente por pausas reales del habla
- ✨ Auto-formato estilo Netflix (2 líneas balanceadas, 42 caracteres)
- 🎬 Highlight de palabra estilo karaoke durante la reproducción
- 💾 Multi-formato: SRT, VTT, ASS/SSA, TXT
- 🌐 99 idiomas soportados
- 🔒 100% offline — tu audio nunca sale de tu computadora

## Descargar

Ve a [Releases](https://github.com/ylimecc/SubFlow/releases/latest) y descarga `SubFlow.exe`. No requiere instalación: doble clic y listo.

Compatible con **Windows 10/11 (64-bit)**.

## Más información

Visita el sitio web: **https://ylimecc.github.io/SubFlow/**
